...
sprite_sheet = pygame.image.load(os.path.join(diretorio_imagens, 'car_yellow.png')).convert_alpha()
...
class Carro(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.imagens_carro = []
        for i in range(2):
            img = sprite_sheet.subsurface((56 * i, 0), (56, 40))
            img = pygame.transform.scale(img, (56 * 3, 40 * 3))
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_a or event.key == K_LEFT:
                carro_pos_x = carro_pos_x - 20
                carro.movimento()
            if event.key == K_d or event.key == K_RIGHT:
                carro_pos_x = carro_pos_x + 20
                carro.movimento()
            if event.key == K_w or event.key == K_UP:
                carro_pos_y = carro_pos_y - 20
                carro.movimento()
