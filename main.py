...
class Carro(pygame.sprite.Sprite): #Este classe vai auxiliar na sprite do carro na tela
    ...
    def __init__(self): 
        sprite_carro = pygame.image.load(os.path.join(configurations.diretorio_imagens, 'car_yellow.png')).convert_alpha()
        pygame.sprite.Sprite.__init__(self)
        self.imagens_carro = []
        for i in range(2):
            img = sprite_carro.subsurface((56*i, 0), (56, 40))
            img = pygame.transform.scale(img, ((configurations.LARGURA//2.7), (configurations.ALTURA//5.5)))
...
class Rua(pygame.sprite.Sprite): #Esta classe vai auxiliar na imagem da tela
    def __init__(self):#Esta função tambem vai auxiliar na imagem de fundo, dando as devidas medidas e a deixando de um modo claro e executável
        sprite_rua = pygame.image.load(os.path.join(
            configurations.diretorio_imagens, 'road.png'))
        pygame.sprite.Sprite.__init__(self)
        self.imagens_rua = []
        for i in range(16):
...
while True: 
    ...
    relogio.tick(30)
    rodovia.draw(configurations.tela)
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN: 
            if event.key == K_a:
                configurations.carro_pos_x = configurations.carro_pos_x - 127
                carro.movimento()
            if event.key == K_LEFT:
                configurations.carro_pos_x = configurations.carro_pos_x - 127
                carro.movimento()
            ...
            if event.key == K_RIGHT:
                ...
                carro.movimento()
            if event.key == K_w:
                configurations.carro_pos_y = configurations.carro_pos_y - configurations.ALTURA//5
                carro.movimento()
            if event.key == K_UP:
                configurations.carro_pos_y = configurations.carro_pos_y - configurations.ALTURA//5
                carro.movimento()
    ...
    todas_as_sprites.draw(configurations.tela) #Este comando auxilia na exibição das sprites na tela
