...
class Buraco(pygame.sprite.Sprite):
    def __init__(self):
        sprite_buraco = pygame.image.load(os.path.join(diretorio_imagens, 'buraco.png')).convert_alpha()
        pygame.sprite.Sprite.__init__(self)
        self.imagens_carro = []
        for i in range(2):
            img = sprite_buraco.subsurface((56 * i, 0), (56, 40))
...
class Carro(pygame.sprite.Sprite):  # Este classe vai auxiliar na sprite do carro na tela
    ...
    def update(
            self):  # Está tambem estará responsavel pela posição do carrinho na tela, aplicando condições especificas
...
class Rua(pygame.sprite.Sprite):  # Esta classe vai auxiliar na imagem da tela
    def __init__(
            self):  # Esta função tambem vai auxiliar na imagem de fundo, dando as devidas medidas e a deixando de um modo claro e executável
        sprite_rua = pygame.image.load(os.path.join(
            diretorio_imagens, 'road.png'))
        pygame.sprite.Sprite.__init__(self)
        self.imagens_rua = []
        for i in range(16):
            img = sprite_rua.subsurface((0, 160 * i), (
            137, 160))  # Esses dois comandos vão dar as escalas, as medidas que apareceram na tela
